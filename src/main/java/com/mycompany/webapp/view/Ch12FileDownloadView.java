...
public class Ch12FileDownloadView extends AbstractView{
	...
	protected void renderMergedOutputModel(Map<String, Object> model, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		//나중에는 DB에서 가져와야할 정보들
		String contentType = request.getServletContext().getMimeType(fileName);  //파일의 확장명에따라 거기에 맞는 파일의 타입을 리턴해준다
		...
		if(file.exists()) {
			// 똑같다 : FileCopyUtils.copy(new FileInputStream(file), response.getOutputStream());
